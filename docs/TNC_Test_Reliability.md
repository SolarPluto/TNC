...
For that reason, handle identity enumeration is retained as an opt-in **local/dev-Windows diagnostic**, not a GitHub Actions job. Normal CI continues to preserve the strict `delta == 0` assertion and the cheap count-persistence samples at T, T+100ms, and T+1000ms. The native-token surface also records cheap `GetProcessHandleCount` deltas at the lifecycle boundaries immediately before inspector construction, immediately after the primary inspect/capture outcome, and immediately after cleanup. When final T is nonzero, those lifecycle deltas are attached to the same diagnostic note, preserving phase attribution even when identity enumeration is disabled.

Setting `TNC_HANDLE_ENUMERATION=1` is appropriate only on a Windows environment where direct timing shows a snapshot cost below **100ms** under representative system load. `SystemExtendedHandleInformation` enumerates the system handle table before the result is filtered to the target PID, so the 26-34 second measurements in run `35341483523` reflect shared-host system-table cost, not the cost of merely walking TNC's ~155 handles; a busier host can be worse.
