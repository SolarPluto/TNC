...
A separate Windows diagnostic CI job enables `TNC_HANDLE_ENUMERATION=1`, enables diagnostic-only soft handling for the strict handle-count assertion, and extends child control/join deadlines to 60 seconds. It runs the native-token test file on pull requests that touch the native token/native pipe implementation or tests, and on manual workflow dispatch. Automatic nightly execution is intentionally disabled while Actions billing/capacity is unresolved; reintroducing periodic sampling is a cost decision, not a correctness requirement.
