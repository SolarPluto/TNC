...
A separate Windows diagnostic CI job enables `TNC_HANDLE_ENUMERATION=1` and extends child control/join deadlines to 60 seconds. On pull requests that touch the native token/native pipe implementation or tests, and on manual workflow dispatch, it runs the native-token test file. A nightly scheduled run on `master` executes the full suite with enumeration enabled so probabilistic handle-count failures can produce survivor evidence without requiring someone to notice and manually rerun a failing PR. A green normal suite validates only the ordinary passing path; a green diagnostic run validates that the opt-in instrumentation executes; a diagnostic run with `T != 0` is the first artifact that validates survivor identity, set algebra, TOKEN metadata, and vanished-handle reporting.
